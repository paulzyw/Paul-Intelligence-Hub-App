...
var gear_train5_explainers;

(function () {

    const scale = (window.devicePixelRatio || 1) > 1.75 ? 2 : 1;

    const neutral_color = rgba255_color(53, 59, 64, 0.20); //240,240,240
    var neutral_outline_color = rgba255_color(192, 198, 204, 1.0); //undefined; // Set to custom color e.g. rgba255_color(255, 255, 255, 0.8) to override default outline color for undefined component colors, If neutral_outline_color is undefined, it automatically falls back to calculating the outline from neutral_color
    ...
    const digit_color = rgba255_color(250, 250, 250, 1);
    const date_ring_color = rgba255_color(30, 33, 36, 1);
    const date_ring_outline_color = rgba255_color(191, 196, 202, 1);
    const hand_color = rgba255_color(250, 250, 250, 1);
    const white_color = rgba255_color(250, 250, 250, 1);
    const dial_color = rgba255_color(39, 42, 45, 1);
    const dial_color_deep_semi = rgba255_color(30, 33, 36, 1);
    const dial_color_deep = rgba255_color(18, 20, 22, 1);
    const dial_outline_color = rgba255_color(191, 196, 202, 1);
    const blue_color_1 = rgba255_color(0, 53, 102, 1.0);
    const blue_color_2 = rgba255_color(0, 119, 182, 1.0);
    const blue_color_3 = rgba255_color(29, 66, 90, 1.0);
    const blue_color_4 = rgba255_color(211, 85, 78, 1.0);
    const blue_color_5 = rgba255_color(212, 175, 55, 1.0);
    const blue_color_6 = rgba255_color(133, 175, 202, 1.0);
    const hero_orange = rgba255_color(237, 137, 54, 1.0);
    const hero_orange_dark_1 = rgba255_color(150, 75, 13, 1.0);
    const hero_golden = rgba255_color(212, 175, 55, 1.0);
    const hero_blue = rgba255_color(0, 163, 224, 1.0);
    const hero_blue_light = rgba255_color(46, 199, 255, 1.0);
    const dial_indicator_color = rgba255_color(250, 250, 250, 1);
    const second_hand_color = rgba255_color(237, 137, 54, 1);
    const main_plate_color = rgba255_color(217, 220, 224, 1.0);
    const pie_1_color = rgba255_color(235, 117, 20, 1.0);
    const pie_2_color = rgba255_color(242, 167, 105, 1.0);
    const pie_3_color = rgba255_color(249, 216, 189, 1.0);

    //const pie_1_color = rgba255_color(50, 117, 205, 1.0);
    //const pie_2_color = rgba255_color(87, 142, 214, 1.0);
    //const pie_3_color = rgba255_color(124, 167, 223, 1.0);

    const red_color = rgba255_color(224, 22, 0, 1);
    ...
    const blue_color_b_1 = rgba255_color(0, 60, 255, 1);
    const violet_color = rgba255_color(212, 148, 224, 1);
    const violet_color_dark = rgba255_color(177, 57, 198, 1);
    ...
    // Default watch pose angles in degrees (X, Y, Z axes)
    // Modify these values to adjust the default initial pose upon loading or refresh:
    var hero_pose_deg_x = 0.0;  // X-axis: Pitch (tilt up / down)
    var hero_pose_deg_y = 0;  // Y-axis: Yaw (turn face left / right)
    var hero_pose_deg_z = -90; // Z-axis: Roll (rotate dial in screen plane)

    function euler_deg_mat3(degX, degY, degZ) {
        let rx = rot_x_mat3(degX * Math.PI / 180);
        let ry = rot_y_mat3(degY * Math.PI / 180);
        let rz = rot_z_mat3(degZ * Math.PI / 180);
    ...
    function GLDrawer(scale, ready_callback) {
        ...
        function mark_ready() { console.log("mark_ready called. v: " + has_vertices + ", i: " + has_indicies + ", b: " + has_basic);
    ...
    `;
        ...
        this.draw_gear_teeth_mesh = function (name, mvp, rotation, color, opacity, shader_type, params, outline_color) {
            ...
            if (color === undefined) {
                color = neutral_color;
                is_undefined_color = true;
            }

            let blend_enabled = false;
            if (color && color[3] !== undefined && color[3] < 1.0) {
                gl.enable(gl.BLEND);
                gl.depthMask(false);
                blend_enabled = true;
            }
            ...
            if (outline_color !== undefined) {
                line_color = outline_color.slice();
            } else if (is_undefined_color && typeof neutral_outline_color !== "undefined" && neutral_outline_color !== null) {
                line_color = neutral_outline_color.slice();
            } else {
                let line_dim = 0.5;
                line_color = color.slice();
        ...
        this.draw_mainspring = function (mvp, rotation, color, end, outline_color) {
        ...
        this.draw_spring = function (mvp, rotation, color, shader_type, params, outline_color) {
        ...
        this.draw_procedural_minute_hand = function (mvp, rotation, color) {
            ...
            rotation = mat3_mul(rotation, rot_z_mat3(-Math.PI / 2));

            if (!this._minute_hand_buffers) {
                let verts = [];
                let metal_tris = [];
                let lume_tris = [];
                let normals = [];

                let addTri = function(p1, p2, p3, isLume) {
                    ...
                    let i = verts.length / 3;
                    verts.push(p1[0], p1[1], p1[2], p2[0], p2[1], p2[2], p3[0], p3[1], p3[2]);
                    normals.push(nx, ny, nz, nx, ny, nz, nx, ny, nz);
                    if (isLume) {
                        lume_tris.push(i, i+1, i+2);
                    } else {
                        metal_tris.push(i, i+1, i+2);
                    }
                };
                ...
                let Ri = 0.5445;
                                                                                                ...
                                                                                                let pts = {
                    C_tail: [0, -2.0, z_edge],
                    C_tail_wide: [0, -1.0, z_ridge],
                    C_pivot_bot: [0, -Ri, z_ridge],
                    C_pivot_top: [0, Ri, z_ridge],
                    C_body_wide: [0, 1.2, z_ridge],
                    C_lume_bot: [0, 3.0, z_ridge],
                    C_lume_top: [0, 9.2, z_ridge],
                    C_tip: [0, 10.8, z_edge],
                    
                    C_lume_bot_in: [0, 3.0, z_lume],
                    C_lume_top_in: [0, 9.2, z_lume],
                    I_lume_bot_in: [0.1815, 3.0, z_lume],
                    I_lume_top_in: [0.0726, 9.2, z_lume],
                    
                    I_lume_bot: [0.1815, 3.0, z_ridge],
                    I_lume_top: [0.0726, 9.2, z_ridge],
                    
                    O_tail: [0, -2.0, z_edge],
                    O_tail_wide: [0.484, -1.0, z_edge],
                    O_pivot_bot: [0.6655, -Ri, z_edge],
                    O_pivot_mid: [0.7865, 0, z_edge],
                    O_pivot_top: [0.6655, Ri, z_edge],
                    O_body_wide: [0.5445, 1.2, z_edge],
                    O_lume_bot: [0.484, 3.0, z_edge],
                    O_lume_top: [0.2178, 9.2, z_edge],
                    O_tip: [0.0968, 10.8, z_edge],
                    
                    H0: [0, -Ri, z_ridge],
                    H1: [Ri*0.707, -Ri*0.707, z_ridge],
                    H2: [Ri, 0, z_ridge],
                    H3: [Ri*0.707, Ri*0.707, z_ridge],
                    H4: [0, Ri, z_ridge]
                };
                ...
                // Top Right
                faces.push( [pts.C_tail, pts.O_tail_wide, pts.C_tail_wide] );
                faces.push( [pts.C_tail_wide, pts.O_tail_wide, pts.O_pivot_bot, pts.H0] );
                faces.push( [pts.H0, pts.O_pivot_bot, pts.H1] );
                faces.push( [pts.H1, pts.O_pivot_bot, pts.O_pivot_mid] );
                faces.push( [pts.H1, pts.O_pivot_mid, pts.H2] );
                faces.push( [pts.H2, pts.O_pivot_mid, pts.H3] );
                faces.push( [pts.H3, pts.O_pivot_mid, pts.O_pivot_top] );
                faces.push( [pts.H3, pts.O_pivot_top, pts.H4] );
                faces.push( [pts.H4, pts.O_pivot_top, pts.O_body_wide, pts.C_body_wide] );
                faces.push( [pts.C_body_wide, pts.O_body_wide, pts.O_lume_bot] );
                faces.push( [pts.C_body_wide, pts.O_lume_bot, pts.I_lume_bot] );
                faces.push( [pts.C_body_wide, pts.I_lume_bot, pts.C_lume_bot] );
                faces.push( [pts.I_lume_bot, pts.O_lume_bot, pts.O_lume_top, pts.I_lume_top] );
                faces.push( [pts.C_lume_top, pts.I_lume_top, pts.O_lume_top] );
                faces.push( [pts.C_lume_top, pts.O_lume_top, pts.O_tip] );
                faces.push( [pts.C_lume_top, pts.O_tip, pts.C_tip] );
                
                for(let i=0; i<faces.length; i++) is_lume_face.push(false);

                // Lume Right
                faces.push( [pts.C_lume_bot_in, pts.I_lume_bot_in, pts.I_lume_top_in, pts.C_lume_top_in] );
                is_lume_face.push(true); // Lume floor
                
                faces.push( [pts.C_lume_bot_in, pts.I_lume_bot_in, pts.I_lume_bot, pts.C_lume_bot] );
                is_lume_face.push(false); // wall
                faces.push( [pts.I_lume_bot_in, pts.I_lume_top_in, pts.I_lume_top, pts.I_lume_bot] );
                is_lume_face.push(false); // wall
                faces.push( [pts.I_lume_top_in, pts.C_lume_top_in, pts.C_lume_top, pts.I_lume_top] );
                is_lume_face.push(false); // wall

                // Perimeter Right
                let perim = [pts.C_tail, pts.O_tail_wide, pts.O_pivot_bot, pts.O_pivot_mid, pts.O_pivot_top, pts.O_body_wide, pts.O_lume_bot, pts.O_lume_top, pts.O_tip, pts.C_tip];
                for (let i = 0; i < perim.length - 1; i++) {
                    faces.push( [[perim[i+1][0], perim[i+1][1], z_bot], [perim[i][0], perim[i][1], z_bot], perim[i], perim[i+1]] );
                    is_lume_face.push(false);
                }

                // Hole Right
                let hole = [pts.H0, pts.H1, pts.H2, pts.H3, pts.H4];
                for (let i = 0; i < hole.length - 1; i++) {
                    faces.push( [[hole[i][0], hole[i][1], z_bot], [hole[i+1][0], hole[i+1][1], z_bot], hole[i+1], hole[i]] );
                    is_lume_face.push(false);
                }

                // Bottom Right
                faces.push( [[pts.C_tail_wide[0], pts.C_tail_wide[1], z_bot], [pts.O_tail_wide[0], pts.O_tail_wide[1], z_bot], [pts.C_tail[0], pts.C_tail[1], z_bot]] );
                faces.push( [[pts.H0[0], pts.H0[1], z_bot], [pts.O_pivot_bot[0], pts.O_pivot_bot[1], z_bot], [pts.O_tail_wide[0], pts.O_tail_wide[1], z_bot], [pts.C_tail_wide[0], pts.C_tail_wide[1], z_bot]] );
                faces.push( [[pts.H1[0], pts.H1[1], z_bot], [pts.O_pivot_bot[0], pts.O_pivot_bot[1], z_bot], [pts.H0[0], pts.H0[1], z_bot]] );
                faces.push( [[pts.O_pivot_mid[0], pts.O_pivot_mid[1], z_bot], [pts.O_pivot_bot[0], pts.O_pivot_bot[1], z_bot], [pts.H1[0], pts.H1[1], z_bot]] );
                faces.push( [[pts.H2[0], pts.H2[1], z_bot], [pts.O_pivot_mid[0], pts.O_pivot_mid[1], z_bot], [pts.H1[0], pts.H1[1], z_bot]] );
                faces.push( [[pts.H3[0], pts.H3[1], z_bot], [pts.O_pivot_mid[0], pts.O_pivot_mid[1], z_bot], [pts.H2[0], pts.H2[1], z_bot]] );
                faces.push( [[pts.O_pivot_top[0], pts.O_pivot_top[1], z_bot], [pts.O_pivot_mid[0], pts.O_pivot_mid[1], z_bot], [pts.H3[0], pts.H3[1], z_bot]] );
                faces.push( [[pts.H4[0], pts.H4[1], z_bot], [pts.O_pivot_top[0], pts.O_pivot_top[1], z_bot], [pts.H3[0], pts.H3[1], z_bot]] );
                faces.push( [[pts.C_body_wide[0], pts.C_body_wide[1], z_bot], [pts.O_body_wide[0], pts.O_body_wide[1], z_bot], [pts.O_pivot_top[0], pts.O_pivot_top[1], z_bot], [pts.H4[0], pts.H4[1], z_bot]] );
                faces.push( [[pts.C_lume_bot[0], pts.C_lume_bot[1], z_bot], [pts.O_lume_bot[0], pts.O_lume_bot[1], z_bot], [pts.O_body_wide[0], pts.O_body_wide[1], z_bot], [pts.C_body_wide[0], pts.C_body_wide[1], z_bot]] );
                faces.push( [[pts.C_lume_top[0], pts.C_lume_top[1], z_bot], [pts.O_lume_top[0], pts.O_lume_top[1], z_bot], [pts.O_lume_bot[0], pts.O_lume_bot[1], z_bot], [pts.C_lume_bot[0], pts.C_lume_bot[1], z_bot]] );
                faces.push( [[pts.C_tip[0], pts.C_tip[1], z_bot], [pts.O_tip[0], pts.O_tip[1], z_bot], [pts.O_lume_top[0], pts.O_lume_top[1], z_bot], [pts.C_lume_top[0], pts.C_lume_top[1], z_bot]] );
                for(let i=0; i<12; i++) is_lume_face.push(false);

                // Add Left Half
                let num_right_faces = faces.length;
                for (let i = 0; i < num_right_faces; i++) {
                    let f = faces[i];
                    let lf = [];
                    for (let j = f.length - 1; j >= 0; j--) {
                        lf.push([-f[j][0], f[j][1], f[j][2]]);
                    }
                    faces.push(lf);
                    is_lume_face.push(is_lume_face[i]);
                }

                // Triangulate all faces
                for (let i=0; i<faces.length; i++) {
                    let f = faces[i];
                    let isLume = is_lume_face[i];
                    if (f.length === 3) {
                        addTri(f[0], f[1], f[2], isLume);
                    } else if (f.length === 4) {
                        addTri(f[0], f[1], f[2], isLume);
                        addTri(f[0], f[2], f[3], isLume);
                    }
                }
                ...
                let ibo_metal = gl.createBuffer();
                gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, ibo_metal);
                gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(metal_tris), gl.STATIC_DRAW);

                let ibo_lume = gl.createBuffer();
                gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, ibo_lume);
                gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(lume_tris), gl.STATIC_DRAW);

                this._minute_hand_buffers = {
                    vertex: vbo,
                    normal: nbo,
                    index_metal: ibo_metal,
                    index_lume: ibo_lume,
                    index_metal_count: metal_tris.length,
                    index_lume_count: lume_tris.length
                };

            }

            
            let b = this._minute_hand_buffers;
            ...
            gl.useProgram(simple_shader.shader);

            gl.bindBuffer(gl.ARRAY_BUFFER, b.vertex);
            gl.enableVertexAttribArray(simple_shader.attributes["v_position"]);
            gl.vertexAttribPointer(simple_shader.attributes["v_position"], 3, gl.FLOAT, false, 0, 0);
            
            gl.bindBuffer(gl.ARRAY_BUFFER, b.normal);
            gl.enableVertexAttribArray(simple_shader.attributes["v_normal"]);
            gl.vertexAttribPointer(simple_shader.attributes["v_normal"], 3, gl.FLOAT, false, 0, 0);
            
            gl.uniformMatrix4fv(simple_shader.uniforms["m_mvp"], false, mat4_transpose(mvp));
            gl.uniformMatrix3fv(simple_shader.uniforms["m_rot"], false, mat3_invert(rotation));

            // Draw metal part
            gl.uniform4fv(simple_shader.uniforms["color"], [color[0], color[1], color[2], 1.0]);
            gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, b.index_metal);
            gl.drawElements(gl.TRIANGLES, b.index_metal_count, gl.UNSIGNED_SHORT, 0);
            
            // Draw lume part
            let lume_color = [0.8, 0.9, 0.8, 1.0]; // Slightly glowing greenish-white
            gl.uniform4fv(simple_shader.uniforms["color"], lume_color);
            gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, b.index_lume);
            gl.drawElements(gl.TRIANGLES, b.index_lume_count, gl.UNSIGNED_SHORT, 0);
        }
        ...
        this.draw_procedural_hour_hand = function (mvp, rotation, color) {
            ...
            if (!this._hour_hand_buffers) {
                ...
                let Ri = 0.572;
                                                                                                                ...
                                                                                                                let pts = {
                    C_tail: [0, -1.84, z_edge],
                    C_tail_wide: [0, -0.92, z_ridge],
                    C_pivot_bot: [0, -Ri, z_ridge],
                    C_pivot_top: [0, Ri, z_ridge],
                    C_body_wide: [0, 1.2, z_ridge],
                    C_lume_bot: [0, 2.4, z_ridge],
                    C_lume_top: [0, 7.3, z_ridge],
                    C_tip: [0, 8.6, z_edge],
                    
                    C_lume_bot_in: [0, 2.4, z_lume],
                    C_lume_top_in: [0, 7.3, z_lume],
                    I_lume_bot_in: [0.187, 2.4, z_lume],
                    I_lume_top_in: [0.077, 7.3, z_lume],
                    
                    I_lume_bot: [0.187, 2.4, z_ridge],
                    I_lume_top: [0.077, 7.3, z_ridge],
                    
                    O_tail: [0, -1.84, z_edge],
                    O_tail_wide: [0.506, -0.92, z_edge],
                    O_pivot_bot: [0.693, -Ri, z_edge],
                    O_pivot_mid: [0.825, 0, z_edge],
                    O_pivot_top: [0.693, Ri, z_edge],
                    O_body_wide: [0.572, 1.2, z_edge],
                    O_lume_bot: [0.506, 2.4, z_edge],
                    O_lume_top: [0.231, 7.3, z_edge],
                    O_tip: [0.099, 8.6, z_edge],
        ...
        this.draw_procedural_second_hand = function (mvp, rotation, color, outline_color) {
            ...
            if (!this._second_hand_buffers) {
                let verts = [];
                let tris = [];

                function addQuad(p1, p2, p3, p4, norm) {
                    let idx = verts.length / 6;
                    verts.push(p1[0], p1[1], p1[2], norm[0], norm[1], norm[2]);
                    verts.push(p2[0], p2[1], p2[2], norm[0], norm[1], norm[2]);
                    verts.push(p3[0], p3[1], p3[2], norm[0], norm[1], norm[2]);
                    verts.push(p4[0], p4[1], p4[2], norm[0], norm[1], norm[2]);

                    tris.push(idx, idx + 1, idx + 2, idx, idx + 2, idx + 3);
                }

                function addTri(p1, p2, p3, norm) {
                    let idx = verts.length / 6;
                    verts.push(p1[0], p1[1], p1[2], norm[0], norm[1], norm[2]);
                    verts.push(p2[0], p2[1], p2[2], norm[0], norm[1], norm[2]);
                    verts.push(p3[0], p3[1], p3[2], norm[0], norm[1], norm[2]);
                ...
                let thickness = 0.35; // slightly thicker to ensure visibility
                let zTop = thickness / 2;
                let zBot = -thickness / 2;
                let normTop = [0, 0, 1];
                let normBot = [0, 0, -1];
                ...
                // 1. Center Hub Annulus Ring (32 segments)
                let segs = 32;
                for (let i = 0; i < segs; i++) {
                    let a1 = (i / segs) * Math.PI * 2;
                    let a2 = ((i + 1) / segs) * Math.PI * 2;
                    let cos1 = Math.cos(a1), sin1 = Math.sin(a1);
                    let cos2 = Math.cos(a2), sin2 = Math.sin(a2);

                    let p1_out_top = [Ro * sin1, Ro * cos1, zTop];
                    let p2_out_top = [Ro * sin2, Ro * cos2, zTop];
                    let p1_in_top  = [Ri * sin1, Ri * cos1, zTop];
                    let p2_in_top  = [Ri * sin2, Ri * cos2, zTop];

                    let p1_out_bot = [Ro * sin1, Ro * cos1, zBot];
                    let p2_out_bot = [Ro * sin2, Ro * cos2, zBot];
                    let p1_in_bot  = [Ri * sin1, Ri * cos1, zBot];
                    let p2_in_bot  = [Ri * sin2, Ri * cos2, zBot];

                    addQuad(p1_in_top, p2_in_top, p2_out_top, p1_out_top, normTop);
                    addQuad(p1_out_bot, p2_out_bot, p2_in_bot, p1_in_bot, normBot);

                    let avg_in_norm = [-(sin1 + sin2) / 2, -(cos1 + cos2) / 2, 0];
                    addQuad(p1_in_top, p1_in_bot, p2_in_bot, p2_in_top, avg_in_norm);

                    let avg_out_norm = [(sin1 + sin2) / 2, (cos1 + cos2) / 2, 0];
                    addQuad(p1_out_top, p2_out_top, p2_out_bot, p1_out_bot, avg_out_norm);
                }

                // 2. Spade / Diamond Contour Profile
                let profile = [
                    // Tail tip to diamond spade counterweight
                    { x: 0.00,  y: -3.60 },
                    { x: 0.15,  y: -3.30 },
                    { x: 0.32,  y: -2.30 }, // Diamond widest point, x:0.50 original value
                    { x: 0.25,  y: -1.40 },
                    { x: 0.15,  y: -0.90 }, // Narrow neck
                    { x: 0.15,  y: -Math.sqrt(Math.max(0, Ro*Ro - 0.15*0.15)) },

                    // Main needle shaft
                    { x: 0.18,  y:  Math.sqrt(Math.max(0, Ro*Ro - 0.18*0.18)) },
                    { x: 0.15,  y:  4.00 },
                    { x: 0.12,  y:  7.00 },
                    { x: 0.08,  y:  9.50 },
                    { x: 0.04,  y: 10.80 },
                    { x: 0.00,  y: 11.50 }  // Main needle tip
                ];

                for (let i = 0; i < profile.length - 1; i++) {
                    if (i === 5) continue; // Skip hub ring hole gap

                    let pt1 = profile[i];
                    let pt2 = profile[i + 1];

                    let r1_top = [ pt1.x, pt1.y, zTop];
                    let l1_top = [-pt1.x, pt1.y, zTop];
                    let r2_top = [ pt2.x, pt2.y, zTop];
                    let l2_top = [-pt2.x, pt2.y, zTop];

                    let r1_bot = [ pt1.x, pt1.y, zBot];
                    let l1_bot = [-pt1.x, pt1.y, zBot];
                    let r2_bot = [ pt2.x, pt2.y, zBot];
                    let l2_bot = [-pt2.x, pt2.y, zBot];

                    if (pt1.x === 0) {
                        addTri(l1_top, r2_top, l2_top, normTop);
                        addTri(l1_bot, l2_bot, r2_bot, normBot);
                    } else if (pt2.x === 0) {
                        addTri(l1_top, r1_top, l2_top, normTop);
                        addTri(l1_bot, l2_bot, r1_bot, normBot);
                    } else {
                        addQuad(l1_top, r1_top, r2_top, l2_top, normTop);
                        addQuad(l1_bot, l2_bot, r2_bot, r1_bot, normBot);
                    }

                    let dx = pt2.x - pt1.x;
                    let dy = pt2.y - pt1.y;
                    let len = Math.hypot(dx, dy) || 1;
                    let rNorm = [dy / len, -dx / len, 0];
                    if (pt1.x > 0 || pt2.x > 0) {
                        addQuad(r1_top, r1_bot, r2_bot, r2_top, rNorm);
                    }

                    let lNorm = [-dy / len, -dx / len, 0];
                    if (pt1.x > 0 || pt2.x > 0) {
                        addQuad(l2_top, l2_bot, l1_bot, l1_top, lNorm);
                    }
                }
            ...
            let buf = this._second_hand_buffers;

            // Solid 3D shaded pass
            gl.disable(gl.BLEND);
            gl.enable(gl.DEPTH_TEST);
            gl.depthMask(true);
            gl.disable(gl.CULL_FACE);
            ...
            gl.enableVertexAttribArray(simple_shader.attributes["v_position"]);
            gl.vertexAttribPointer(simple_shader.attributes["v_position"], 3, gl.FLOAT, false, 24, 0);
            gl.enableVertexAttribArray(simple_shader.attributes["v_normal"]);
            gl.vertexAttribPointer(simple_shader.attributes["v_normal"], 3, gl.FLOAT, false, 24, 12);

            gl.uniformMatrix4fv(simple_shader.uniforms["m_mvp"], false, mat4_transpose(mvp));
            gl.uniformMatrix3fv(simple_shader.uniforms["m_rot"], false, mat3_invert(rotation));
            gl.uniform4fv(simple_shader.uniforms["color"], color || [0.93, 0.54, 0.21, 1.0]);
    ...
    function Drawer(container, mode) {
        ...
        wrapper.classList.add("canvas_container"); wrapper.style.width = "100%"; wrapper.style.height = "100%"; wrapper.style.position = "absolute"; wrapper.style.top = "0"; wrapper.style.left = "0";
        ...
        this.loading_div.textContent = "Loading..."; this.loading_div.style.position = "absolute"; this.loading_div.style.top = "50%"; this.loading_div.style.left = "50%"; this.loading_div.style.transform = "translate(-50%, -50%)"; this.loading_div.style.color = "white";
        ...
        if (mode === "hero_movement") {
            ...
            rot = euler_deg_mat3(hero_pose_deg_x, hero_pose_deg_y, hero_pose_deg_z);
        ...
        this.reset_pose = function () {
            if (mode === "hero") {
                this.set_rot(euler_deg_mat3(hero_pose_deg_x, hero_pose_deg_y, hero_pose_deg_z));
                if (this.sliders) {
        ...
        this.set_pose_x = function (deg_x) {
            if (mode === "hero") {
                this.set_rot(euler_deg_mat3(deg_x, hero_pose_deg_y, hero_pose_deg_z));
            }
        }

        this.destroy = function() {
            self.set_paused(true);
            window.removeEventListener("resize", self.on_resize, true);
            window.removeEventListener("load", self.on_resize, true);
            
            let animIdx = animated_drawers.indexOf(self);
            if (animIdx !== -1) {
                animated_drawers.splice(animIdx, 1);
            }
            let drawIdx = all_drawers.indexOf(self);
            if (drawIdx !== -1) {
                all_drawers.splice(drawIdx, 1);
            }
            let contIdx = all_containers.indexOf(container);
            if (contIdx !== -1) {
                all_containers.splice(contIdx, 1);
            }
            
            container.innerHTML = "";
            delete container.drawer;
        };
        ...
        this.repaint = function (dt) {
            ...
            if (mode === "hero") {
                ...
                {
                    colors["Hour_wheel"] = blue_color_2;
                    colors["Date_indicator_gear"] = blue_color_1;
                    colors["Date_indicator_cover"] = blue_color_1;
                    colors["Date_indicator_spring"] = blue_color_2;
                    colors["Date_jumper_gears"] = blue_color_2;
                    colors["Date_jumper"] = blue_color_3;
                    //colors["Weight"] = rgba255_color(53, 59, 64, 0.3);  
                    //colors["Weight_outline"] = rgba255_color(255, 255, 255, 1.0);                  
                    colors["Mainplate"] = main_plate_color;
                    colors["First"] = hero_orange;
                    colors["Winding_pinion"] = blue_color_3;
                                                            
                    colors["Cannon_wheel"] = blue_color_2;
                    colors["Cannon_pinion"] = blue_color_3;
                    colors["Intermediate_wheel"] = blue_color_5;
                    colors["Click"] = blue_color_3;
                    colors["Crown_wheel"] = blue_color_2;
                    colors["Click_spring"] = blue_color_3;
                    colors["Ratchet_wheel"] = blue_color_1;
                    colors["Stop_lever"] = blue_color_3;
                    colors["Second"] = blue_color_2;
                    colors["Setting_wheel"] = blue_color_2;
                    colors["Setting_lever"] = blue_color_3;
                    colors["Third"] = blue_color_3; //blue_color_1;
                    colors["Barrel_arbor"] = blue_color_6;
                    colors["Corrector_lever"] = blue_color_4;
                    colors["Fourth"] = hero_golden;//blue_color_3;

                    colors["Escape"] = blue_color_3;
                    colors["Winding_pinion"] = blue_color_3;
                    colors["Yoke"] = blue_color_3;
                    colors["Stop_lever"] = blue_color_4;
                   
                    colors["Balance_wheel"] = hero_blue;
                    colors["Hairspring"] = hero_blue;
                    colors["Setting_lever_jumper"] = blue_color_2;
                }
                ...
                draw_complete_assembly(mvp, rot, 200, colors, {
                    ...
                    "balance_time": sim[0],
                    "first_wheel_angle": sim[0] * 0.15,
                    "cannon_wheel_angle": -sim[0] * 0.15,
                    "ratchet_wheel_angle": sim[0] * 0.15,
                    "second_wheel_angle": -sim[0] * 0.15,
                    "third_wheel_angle": sim[0] * 0.15,
                    "show_date_ring": true,
                ...
                if (mode !== "gear_train") {
                    let m = mat4_mul(mat4_mul(mvp, translation_mat4([0, 0, 124.8])), rot_z_mat4(-escape_angle * w50_n / w41_n + 1.4));
                    gl.draw_procedural_second_hand(m, rot, hand_color);
                }
                ...
                if (mode === "mainspring_wind2") {
                    ...
                    gl.draw_procedural_minute_hand(mat4_mul(mvp, translation_mat4([0, 0, 122.6])), r, hand_color);
            ...
            function draw_automatic(mvp, rot, weight_angle, t0, t1, p, colors, offsets, skip_screws, dot) {
...

                {
                    ...
                    if (!offset("skip_weight"))
                        gl.draw_mesh("Weight", m, r, colors["Weight"], undefined, undefined, undefined, colors["Weight_outline"]);
            ...
            function draw_complete_assembly(mvp, rot, step, colors = {}, offsets = {}, params = {}) {
                ...
                if (params && params["ratchet_wheel_angle"] !== undefined) {
                    arbor_angle = params["ratchet_wheel_angle"];
                } else if (params && params["arbor"] !== undefined) {
                ...
                function draw_mainplate(mvp, rot, color) {
                    gl.draw_mesh("Mainplate", mvp, rot, color, undefined, undefined, undefined, colors["Mainplate_outline"]);
                ...
                if (step >= 10) {
                    let first_wheel_angle = (params && params["first_wheel_angle"] !== undefined) ?
                        params["first_wheel_angle"] :
                        (escape_angle * w50_n / w41_n * w40_n / w31_n * w30_n / w21_n * w20_n / w11_n + 0.002);
                    draw_first_wheel(mat4_mul(mvp, translation_mat4([0, 0, offset("First")])), rot, first_wheel_angle,
                        offsets["Cut_barrel"], false,
                        colors["First"]);
                    let second_wheel_angle = (params && params["second_wheel_angle"] !== undefined) ?
                        params["second_wheel_angle"] :
                        (-escape_angle * w50_n / w41_n * w40_n / w31_n * w30_n / w21_n + 0.0495);
                    draw_second_wheel(mat4_mul(mvp, translation_mat4([0, 0, offset("Second")])), rot, second_wheel_angle, colors["Second"]);
                    let third_wheel_angle = (params && params["third_wheel_angle"] !== undefined) ?
                        params["third_wheel_angle"] :
                        (escape_angle * w50_n / w41_n * w40_n / w31_n + 0.041);
                    draw_third_wheel(mat4_mul(mvp, translation_mat4([0, 0, offset("Third")])), rot, third_wheel_angle, colors["Third"]);
                ...
                if (step >= 45) {
                    ...
                    let cw_angle = (params && params["cannon_wheel_angle"] !== undefined) ?
                        params["cannon_wheel_angle"] :
                        (-escape_angle * w50_n / w41_n * w40_n / w31_n * w30_n / wc0_n + 0.063);

                    draw_cannon_wheel(mat4_mul(mvp, translation_mat4([0, 0, offset("Cannon_wheel")])), rot, cw_angle, colors["Cannon_wheel"]);
                ...
                if (params["show_second_hand"]) {
                    ...
                    gl.draw_procedural_second_hand(m, r, second_hand_color);
                }
                ...
                if (params["show_minute_hand"]) {
                    ...
                    gl.draw_procedural_minute_hand(m, r, hand_color);
                }
                ...
                if (params["show_hour_hand"]) {
                    ...
                    gl.draw_procedural_hour_hand(m, r, hand_color);
                }
                if (step >= 0) {
            ...
            function draw_date_ring(mvp, rot, angle, offset) {
                ...
                gl.draw_mesh("Date_ring", mvp, rot, date_ring_color, 1, undefined, undefined, date_ring_outline_color);
            ...
            function draw_dial(mvp, rot) {
                ...
                gl.draw_mesh("Dial", mat4_mul(mvp, translation_mat4([0, 0, -29.7])), rot, dial_color, 1, undefined, undefined, dial_outline_color);
                ...
                let draw_dial_ring_1_canvas_texture = (outerDiameter, innerDiameter, fillColor, outlineColor, outlineLineWidth = 3, px = 0.0, py = 0.0) => {
                    ...
                    // Initialize Texture Quad Shader
                    if (!window._text_texture_shader) {
                        let vert_src = `
                        ...
                        `;
                        let shader = new Shader(rawGL, vert_src, frag_src, ["a_position", "a_texcoord"], ["u_mvp", "u_texture"]);
                        ...
                        let vbo = rawGL.createBuffer();
                        rawGL.bindBuffer(rawGL.ARRAY_BUFFER, vbo);
                        rawGL.bufferData(rawGL.ARRAY_BUFFER, quad_verts, rawGL.STATIC_DRAW);

                        window._text_texture_shader = { shader, vbo };
                    }

                    let parseColor = (c) => typeof c === "string" ? c : (c ? rgba_color_string(c) : "rgba(0,0,0,0)");
                    let fillStr = parseColor(fillColor);
                    let outlineStr = parseColor(outlineColor);

                    if (!window._dial_ring_canvas_map) window._dial_ring_canvas_map = {};
                    let cacheKey = `ring1_${outerDiameter}_${innerDiameter}_${fillStr}_${outlineStr}_${outlineLineWidth}_nocutout`;
                    let ringObj = window._dial_ring_canvas_map[cacheKey];
                    ...
                    if (!ringObj) {
                        ...
                        let cx = resolution / 2;
                        let cy = resolution / 2;
                        let outerR = (resolution / 2) - outlineLineWidth - 2;
                        let innerR = outerR * (innerDiameter / outerDiameter);

                        // Fill the annular ring (donut shape)
                        ctx.beginPath();
                        ctx.arc(cx, cy, outerR, 0, Math.PI * 2, false);
                        ctx.arc(cx, cy, innerR, 0, Math.PI * 2, true); // Counter-clockwise for hole cutout
                        ctx.fillStyle = fillStr;
                        ctx.fill();

                        // Draw inner & outer outline rings
                        if (outlineColor && outlineLineWidth > 0) {
                        ...
                        let tex = rawGL.createTexture();
                        rawGL.bindTexture(rawGL.TEXTURE_2D, tex);
                        rawGL.texImage2D(rawGL.TEXTURE_2D, 0, rawGL.RGBA, rawGL.RGBA, rawGL.UNSIGNED_BYTE, canvas);
                        rawGL.texParameteri(rawGL.TEXTURE_2D, rawGL.TEXTURE_MIN_FILTER, rawGL.LINEAR);
                        rawGL.texParameteri(rawGL.TEXTURE_2D, rawGL.TEXTURE_MAG_FILTER, rawGL.LINEAR);
                        rawGL.texParameteri(rawGL.TEXTURE_2D, rawGL.TEXTURE_WRAP_S, rawGL.CLAMP_TO_EDGE);
                        rawGL.texParameteri(rawGL.TEXTURE_2D, rawGL.TEXTURE_WRAP_T, rawGL.CLAMP_TO_EDGE);

                        ringObj = { canvas, tex };
                        window._dial_ring_canvas_map[cacheKey] = ringObj;
                    }

                    let s = window._text_texture_shader;
                    if (!s) return;
                    rawGL.useProgram(s.shader.shader);

                    let m = scale_mat4([outerDiameter, outerDiameter, 1.0]);
                    m = mat4_mul(translation_mat4([px, py, 2.65]), m);
                    let final_mvp = mat4_mul(mvp, m);

                    rawGL.uniformMatrix4fv(s.shader.uniforms["u_mvp"], false, mat4_transpose(final_mvp));

                    rawGL.activeTexture(rawGL.TEXTURE0);
                    rawGL.bindTexture(rawGL.TEXTURE_2D, ringObj.tex);
                    rawGL.uniform1i(s.shader.uniforms["u_texture"], 0);

                    rawGL.bindBuffer(rawGL.ARRAY_BUFFER, s.vbo);
                    let stride = 5 * 4;
                    rawGL.enableVertexAttribArray(s.shader.attributes["a_position"]);
                    rawGL.vertexAttribPointer(s.shader.attributes["a_position"], 3, rawGL.FLOAT, false, stride, 0);

                    rawGL.enableVertexAttribArray(s.shader.attributes["a_texcoord"]);
                    rawGL.vertexAttribPointer(s.shader.attributes["a_texcoord"], 2, rawGL.FLOAT, false, stride, 3 * 4);

                    rawGL.disable(rawGL.CULL_FACE);
                    rawGL.enable(rawGL.BLEND);
                    rawGL.blendFunc(rawGL.SRC_ALPHA, rawGL.ONE_MINUS_SRC_ALPHA);
                    rawGL.depthMask(false);

                    rawGL.drawArrays(rawGL.TRIANGLE_STRIP, 0, 4);

                    rawGL.depthMask(true);
                };

                // Draw Dial Ring on dial surface:
                let dial_ring_1_outer_diameter = 26.15; // Outer diameter (matches dial outer boundary)
                let dial_ring_1_inner_diameter = 23.7; // Inner diameter (easily adjustable)
                let dial_ring_1_outline_width = 3;     // Outline width in pixels
                draw_dial_ring_1_canvas_texture(dial_ring_1_outer_diameter, dial_ring_1_inner_diameter, dial_color_deep_semi, dial_outline_color, dial_ring_1_outline_width);
                ...
                let draw_dial_ring_2_canvas_texture = (outerDiameter, innerDiameter, fillColor, outlineColor, outlineLineWidth = 3, px = 0.0, py = 0.0) => {
                    ...
                    if (!window._dial_ring_canvas_map) window._dial_ring_canvas_map = {};
                    let cacheKey = `ring2_${outerDiameter}_${innerDiameter}_${fillStr}_${outlineStr}_${outlineLineWidth}_cutout12oClock_v4correctedPositions`;
                    let ringObj = window._dial_ring_canvas_map[cacheKey];
                    ...
                    if (!ringObj) {
                        ...
                        // =========================================================================
                        // Draw numbers (60, 5, 10, ... 55) on Dial Ring 2 surface
                        // =========================================================================
                        let numFontSize = 30;                            // Height / base font size in pixels
                        let numWidthScale = 1.0;                         // Adjustable width scale
                        let numHeightScale = 0.85;                        // Adjustable height scale
                        let numRadiusOffset = 0;                         // Radial offset (+ for outward, - for inward)
                        let numRadius = ((innerR + outerR) / 2) + numRadiusOffset;
                        let numColor = typeof dial_indicator_color === "string" ? dial_indicator_color : rgba_color_string(dial_indicator_color);

                        ctx.save();
                        ctx.font = `400 ${numFontSize}px 'Segoe UI', Arial, sans-serif`;
                        ctx.fillStyle = numColor;
                        ctx.textAlign = "center";
                        ctx.textBaseline = "middle";

                        const ringNumbers = [
                            { text: "60", hour: 9,  dir: "down" }, // 9 o'clock
                            { text: "5",  hour: 10, dir: "down" }, // 10 o'clock
                            { text: "10", hour: 11, dir: "down" }, // 11 o'clock
                            { text: "15", hour: 12, dir: "down" }, // 12 o'clock
                            { text: "20", hour: 1,  dir: "up" },   // 13 o'clock (1 o'clock)
                            { text: "25", hour: 2,  dir: "up" },   // 14 o'clock (2 o'clock)
                            { text: "30", hour: 3,  dir: "up" },   // 15 o'clock (3 o'clock)
                            { text: "35", hour: 4,  dir: "up" },   // 16 o'clock (4 o'clock)
                            { text: "40", hour: 5,  dir: "up" },   // 17 o'clock (5 o'clock)
                            { text: "45", hour: 6,  dir: "down" }, // 18 o'clock (6 o'clock)
                            { text: "50", hour: 7,  dir: "down" }, // 19 o'clock (7 o'clock)
                            { text: "55", hour: 8,  dir: "down" }  // 20 o'clock (8 o'clock)
                        ];

                        for (let item of ringNumbers) {
                            // Hour H mapped to canvas angle: (H - 3) * 30 degrees
                            let angleRad = ((item.hour - 3) * 30) * (Math.PI / 180);
                            let tx = cx + Math.cos(angleRad) * numRadius;
                            let ty = cy + Math.sin(angleRad) * numRadius;

                            ctx.save();
                            ctx.translate(tx, ty);

                            let rotAngle = item.dir === "down" ? angleRad + Math.PI / 2 : angleRad - Math.PI / 2;
                            ctx.rotate(rotAngle);
                            ctx.scale(numWidthScale, numHeightScale);
                        ...
                        // Cut out rectangular hole at 12 o'clock position (top side of ring)
                        let holeWidth = 90;
                        let holeHeight = 60;
                        let holeX = cx - holeWidth / 2;
                        let holeY = cy - outerR - -10;
                ...
                // Draw Dial Ring on dial surface:
                let dial_ring_2_outer_diameter = 23.68; // Outer diameter (matches dial outer boundary)
                let dial_ring_2_inner_diameter = 22.2; // Inner diameter (easily adjustable)
                let dial_ring_2_outline_width = 0;     // Outline width in pixels
                draw_dial_ring_2_canvas_texture(dial_ring_2_outer_diameter, dial_ring_2_inner_diameter, dial_color_deep, dial_outline_color, dial_ring_2_outline_width);
                    ...
                    if (!window._prism_shader) {
                        ...
                        let frag_src = `
                            ...
                            void main(void) {
                                ...
                                float diff = max(dot(N, L), 0.35);
                                gl_FragColor = vec4(u_color.rgb * diff, u_color.a);
                            }
                        `;
                        let shader = new Shader(rawGL, vert_src, frag_src, ["a_position", "a_normal"], ["u_mvp", "u_rot", "u_color"]);

                        let nL_x = -0.8944, nL_y = -0.4472;
                        let nR_x = 0.8944, nR_y = -0.4472;
                        let vertices = new Float32Array([
                            // Top face (Z = +0.5, N = 0, 0, 1)
                            -0.5,  0.5,  0.5,   0, 0, 1,
                             0.5,  0.5,  0.5,   0, 0, 1,
                             0.0, -0.5,  0.5,   0, 0, 1,

                            // Bottom face (Z = -0.5, N = 0, 0, -1)
                            -0.5,  0.5, -0.5,   0, 0, -1,
                             0.0, -0.5, -0.5,   0, 0, -1,
                             0.5,  0.5, -0.5,   0, 0, -1,

                            // Back face (Y = 0.5, N = 0, 1, 0)
                            -0.5,  0.5, -0.5,   0, 1, 0,
                             0.5,  0.5, -0.5,   0, 1, 0,
                             0.5,  0.5,  0.5,   0, 1, 0,
                            -0.5,  0.5,  0.5,   0, 1, 0,
                            ...
                            // Left face (N_L)
                            -0.5,  0.5, -0.5,   nL_x, nL_y, 0,
                            -0.5,  0.5,  0.5,   nL_x, nL_y, 0,
                             0.0, -0.5,  0.5,   nL_x, nL_y, 0,
                             0.0, -0.5, -0.5,   nL_x, nL_y, 0,

                            // Right face (N_R)
                             0.5,  0.5, -0.5,   nR_x, nR_y, 0,
                             0.0, -0.5, -0.5,   nR_x, nR_y, 0,
                             0.0, -0.5,  0.5,   nR_x, nR_y, 0,
                             0.5,  0.5,  0.5,   nR_x, nR_y, 0
                        ]);
                        ...
                        let vbo = rawGL.createBuffer();
                        rawGL.bindBuffer(rawGL.ARRAY_BUFFER, vbo);
                        rawGL.bufferData(rawGL.ARRAY_BUFFER, vertices, rawGL.STATIC_DRAW);

                        let ibo = rawGL.createBuffer();
                        rawGL.bindBuffer(rawGL.ELEMENT_ARRAY_BUFFER, ibo);
                        rawGL.bufferData(rawGL.ELEMENT_ARRAY_BUFFER, indices, rawGL.STATIC_DRAW);

                        window._prism_shader = { shader, vbo, ibo };
                    }

                    let s = window._prism_shader;
                    rawGL.useProgram(s.shader.shader);

                    rawGL.bindBuffer(rawGL.ARRAY_BUFFER, s.vbo);
                    rawGL.bindBuffer(rawGL.ELEMENT_ARRAY_BUFFER, s.ibo);

                    let posLoc = s.shader.attributes["a_position"];
                    let normLoc = s.shader.attributes["a_normal"];
                    rawGL.enableVertexAttribArray(posLoc);
                    rawGL.vertexAttribPointer(posLoc, 3, rawGL.FLOAT, false, 24, 0);
                    rawGL.enableVertexAttribArray(normLoc);
                    rawGL.vertexAttribPointer(normLoc, 3, rawGL.FLOAT, false, 24, 12);

                    rawGL.uniformMatrix4fv(s.shader.uniforms["u_mvp"], false, mat4_transpose(mvp));
                    rawGL.uniformMatrix3fv(s.shader.uniforms["u_rot"], false, mat3_transpose(rotation));
                    rawGL.uniform4fv(s.shader.uniforms["u_color"], color);

                    rawGL.disable(rawGL.CULL_FACE);
                    rawGL.drawElements(rawGL.TRIANGLES, 24, rawGL.UNSIGNED_SHORT, 0);
                };
                ...
                for (let i = 0; i < 60; i++) {
                    ...
                    let l = is_hour_tick ? 1.0 : 0.35;
                    let w = is_hour_tick ? 0.9 : 0.1;
                    let m = rot_z_mat4(i * 2 * pi / 60);
                    m = mat4_mul(m, translation_mat4([0, 12.85 - l / 2, 2.6]));
                    m = mat4_mul(m, scale_mat4([w, l, 0.1]));

                    let r = rot_z_mat3(i * 2 * pi / 60);

                    if (is_hour_tick) {
                        draw_triangular_prism(mat4_mul(mvp, m), mat3_mul(rot, r), dial_indicator_color);
                    } else {
                        gl.draw_mesh("Cube", mat4_mul(mvp, m), mat3_mul(rot, r), dial_indicator_color, 1, "no_lines"); // dial indicator drawing
                    }
                ...
                let draw_revos_canvas_text = (textString, px, py, width, height, self_angle_deg = 0, dial_center_angle_deg = 0) => {
                    ...
                    if (!window._text_texture_shader) {
                        ...
                        // 2D Quad geometry centered at (0, 0, 0)
                        let quad_verts = new Float32Array([
                            -0.5, -0.5, 0.0,  0.0, 1.0,
                             0.5, -0.5, 0.0,  1.0, 1.0,
                            -0.5,  0.5, 0.0,  0.0, 0.0,
                    ...
                    // Render or retrieve cached 2D Canvas texture for textString
                    if (!window._text_canvas_map) window._text_canvas_map = {};
                    let colorStr = typeof dial_indicator_color === "string" ? dial_indicator_color : rgba_color_string(dial_indicator_color); // test color (dial_indicator_color, or rgba(250, 250, 250, 1.0))
                    let cacheKey = textString + "_" + colorStr;
                    let textObj = window._text_canvas_map[cacheKey];
                    ...
                    if (!textObj) {
                        ...
                        textObj = { canvas, tex };
                        window._text_canvas_map[cacheKey] = textObj;
                    }
                    ...
                    // 1. Scale quad to width & height
                    let m = scale_mat4([width, height, 1.0]);

                    // 2. Rotate around text's own center
                    if (self_angle_deg !== 0) {
                        let rad_self = self_angle_deg * Math.PI / 180;
                        m = mat4_mul(rot_z_mat4(rad_self), m);
                    }

                    // 3. Translate to (px, py, Z=2.65) on dial face
                    m = mat4_mul(translation_mat4([px, py, 2.65]), m);

                    // 4. Rotate around the dial center (0, 0) in Z-axis
                    if (dial_center_angle_deg !== 0) {
                        let rad_dial = dial_center_angle_deg * Math.PI / 180;
                        m = mat4_mul(rot_z_mat4(rad_dial), m);
                    }

                    let quad_mvp = mat4_mul(mvp, m);

                    rawGL.uniformMatrix4fv(s.shader.uniforms["u_mvp"], false, mat4_transpose(quad_mvp));

                    rawGL.activeTexture(rawGL.TEXTURE0);
                    rawGL.bindTexture(rawGL.TEXTURE_2D, textObj.tex);
                    rawGL.uniform1i(s.shader.uniforms["u_texture"], 0);
                ...
                let text_revos_self_angle_deg = 180;        // e.g. 0, 90, 180, -90
                let dial_revos_center_angle_deg = -90;      // e.g. 0, 90 (3 o'clock), 180 (12 o'clock), -90 (9 o'clock)
                draw_revos_canvas_text("REVOS", -6.0, -0.335, 20.0, 4.0, text_revos_self_angle_deg, dial_revos_center_angle_deg); // parameter note: (px, py, width, height, self_angle_deg, dial_center_angle_deg)
                ...
                let draw_pw_canvas_text = (textString, px, py, width, height, self_angle_deg = 0, dial_center_angle_deg = 0) => {
                    ...
                    // Render or retrieve cached 2D Canvas texture for textString
                    if (!window._text_canvas_map) window._text_canvas_map = {};
                    let colorStr = typeof dial_indicator_color === "string" ? dial_indicator_color : rgba_color_string(dial_indicator_color);
                    let cacheKey = textString + "_" + colorStr + "_font390";
                    let textObj = window._text_canvas_map[cacheKey];
                ...
                let draw_pie_chart_1_canvas_texture = (targetPercentage, px, py, width, height, self_angle_deg = 0, dial_center_angle_deg = 0, anim_duration_sec = 2.0, active_line_width = 26, track_line_width = 22, start_delay_sec = 0.0) => {
                    ...
                    // Initialize Texture Quad Shader if not already initialized
                    if (!window._text_texture_shader) {
                        let vert_src = `
                            attribute vec3 a_position;
                            attribute vec2 a_texcoord;
                    ...
                    // Animation progress calculation (from 0% to targetPercentage over anim_duration_sec)
                    if (!window._pie_chart_1_start_time) {
                        window._pie_chart_1_start_time = Date.now();
                    }
                    if (!window._pie_chart_shared_start_time) {
                        window._pie_chart_shared_start_time = Date.now();
                    }
                    let elapsed = (Date.now() - window._pie_chart_shared_start_time) / 1000;
                    let loop_cycle_sec = 22.0; // Repeat every 22 seconds
                    let time_in_current_loop = elapsed % loop_cycle_sec;
                    
                    let time_since_start = Math.max(0, time_in_current_loop - (start_delay_sec || 0.0));
                    let duration = anim_duration_sec || 2.0; // Animation duration in seconds
                    let animProgress = Math.min(1.0, time_since_start / duration);
                    // Smooth ease-out cubic curve
                    let easeProgress = 1.0 - Math.pow(1.0 - animProgress, 3);
                    let currentPercentage = targetPercentage * easeProgress;

                    // Retrieve or create canvas/texture object
                    if (!window._pie_chart_1_obj) {
                        let canvas = document.createElement("canvas");
                        canvas.width = 512;
                        canvas.height = 512;
                        ...
                        window._pie_chart_1_obj = { canvas, tex, lastVal: -1 };
                    }

                    let pieObj = window._pie_chart_1_obj;
                    let colorStr = typeof pie_1_color === "string" ? pie_1_color : rgba_color_string(pie_1_color);

                    // Redraw canvas and update GL texture when value changes during animation
                    let currentDisplayVal = Math.round(currentPercentage * 10) / 10;
                    if (pieObj.lastVal !== currentDisplayVal) {
                        pieObj.lastVal = currentDisplayVal;

                        let canvas = pieObj.canvas;
                        let ctx = canvas.getContext("2d");
                        ctx.clearRect(0, 0, 512, 512);

                        let cx = 256, cy = 256, r = 180;
                        let trackLineWidth = track_line_width;
                        let activeLineWidth = active_line_width;

                        // Draw radial stripes from center to outside
                        let total_stripes = 60;     // Configurable density (default 160 stripes in 360 degree) //110
                        let stripe_thickness = 8;  // Thickness of each stripe //3.5

                        ctx.lineCap = "round";
                        ctx.lineWidth = stripe_thickness;

                        for (let i = 0; i < total_stripes; i++) {
                            let angle = -Math.PI / 2 + (i / total_stripes) * 2 * Math.PI;
                            let stripePercent = (i / total_stripes) * 100;
                            let isActive = (stripePercent <= currentPercentage) && (currentPercentage > 0);

                            let r_inner, r_outer, strokeColor;
                            if (isActive) {
                                r_inner = r - activeLineWidth / 2;
                                r_outer = r + activeLineWidth / 2;
                                strokeColor = colorStr;
                            } else {
                                r_inner = r - trackLineWidth / 2;
                                r_outer = r + trackLineWidth / 2;
                                strokeColor = "rgba(250, 250, 250, 0.1)";
                            }

                            let cos = Math.cos(angle);
                            let sin = Math.sin(angle);
                            let x_start = cx + r_inner * cos;
                            let y_start = cy + r_inner * sin;
                            let x_end = cx + r_outer * cos;
                            let y_end = cy + r_outer * sin;
                        ...
                        ctx.fillStyle = rgba_color_string(dial_indicator_color);//colorStr;
                        ctx.textAlign = "center";
                        ctx.textBaseline = "middle";
                        ctx.fillText(Math.round(currentPercentage) + "%", cx, cy);

                        // Upload canvas to WebGL texture
                        rawGL.bindTexture(rawGL.TEXTURE_2D, pieObj.tex);
                        rawGL.texImage2D(rawGL.TEXTURE_2D, 0, rawGL.RGBA, rawGL.RGBA, rawGL.UNSIGNED_BYTE, canvas);
                    }
                    ...
                    // 2. Rotate around chart's own center
                    if (self_angle_deg !== 0) {
                        let rad_self = self_angle_deg * Math.PI / 180;
                        m = mat4_mul(rot_z_mat4(rad_self), m);
                    }
                ...
                // Draw 60% Animated Pie Chart at 9 o'clock position (between dial center and 9 o'clock hour indicator):
                let pie_1_target_pct = 86;
                let pie_1_px = -6.0;                  // Position X (at 9 o'clock side)
                let pie_1_py = 0.0;                   // Position Y
                let pie_1_width = 8.5;               // Chart width
                let pie_1_height = 8.5;              // Chart height
                let pie_1_self_angle_deg = 90;        // Rotation around chart's own center
                let pie_1_dial_center_angle_deg = 0; // Rotation around center of dial (Z-axis)
                let pie_1_anim_duration = 6.6;        // Animation duration in seconds (e.g., 2.0s, 3.5s, 0.5s)
                let pie_1_active_line_width = 35;     // Width of the active doughnut bar arc (increase e.g. 36 or 44 for thicker bar)
                let pie_1_track_line_width = 31;      // Width of the background track ring
                let pie_1_start_delay = 0.0;          // Start delay in seconds in 22s loop cycle
                try { draw_pie_chart_1_canvas_texture(pie_1_target_pct, pie_1_px, pie_1_py, pie_1_width, pie_1_height, pie_1_self_angle_deg, pie_1_dial_center_angle_deg, pie_1_anim_duration, pie_1_active_line_width, pie_1_track_line_width, pie_1_start_delay); } catch(e) { console.error('Error in pie 1:', e); }
                ...
                let draw_pie_chart_2_canvas_texture = (targetPercentage, px, py, width, height, self_angle_deg = 0, dial_center_angle_deg = 0, anim_duration_sec = 2.0, active_line_width = 26, track_line_width = 22, start_delay_sec = 0.0) => {
                    ...
                    if (!window._pie_chart_2_obj) {
                        ...
                        window._pie_chart_2_obj = { canvas, tex, lastVal: -1 };
                    }

                    let pieObj = window._pie_chart_2_obj;
                    let colorStr = typeof pie_2_color === "string" ? pie_2_color : rgba_color_string(pie_2_color);
                    ...
                    if (pieObj.lastVal !== currentDisplayVal) {
                        ...
                        // Draw radial stripes from center to outside
                        let total_stripes = 60;     // Configurable density (default 160 stripes in 360 degree)
                        let stripe_thickness = 8.0;  // Thickness of each stripe
                ...
                // Draw 60% Animated Pie Chart at 6 o'clock position (between dial center and 9 o'clock hour indicator):
                let pie_2_target_pct = 68;
                let pie_2_px = 0.0;                  // Position X (at 6 o'clock side)
                let pie_2_py = -6.0;                   // Position Y
                let pie_2_width = 8.5;               // Chart width
                let pie_2_height = 8.5;              // Chart height
                let pie_2_self_angle_deg = 90;        // Rotation around chart's own center
                let pie_2_dial_center_angle_deg = 0; // Rotation around center of dial (Z-axis)
                let pie_2_anim_duration = 6.6;        // Animation duration in seconds (e.g., 2.0s, 3.5s, 0.5s)
                let pie_2_active_line_width = 35;     // Width of the active doughnut bar arc (increase e.g. 36 or 44 for thicker bar)
                let pie_2_track_line_width = 31;      // Width of the background track ring
                let pie_2_start_delay = 3.0;          // Start delay in seconds in 22s loop cycle
                try { draw_pie_chart_2_canvas_texture(pie_2_target_pct, pie_2_px, pie_2_py, pie_2_width, pie_2_height, pie_2_self_angle_deg, pie_2_dial_center_angle_deg, pie_2_anim_duration, pie_2_active_line_width, pie_2_track_line_width, pie_2_start_delay); } catch(e) { console.error('Error in pie 2:', e); }
                ...
                let draw_pie_chart_3_canvas_texture = (targetPercentage, px, py, width, height, self_angle_deg = 0, dial_center_angle_deg = 0, anim_duration_sec = 2.0, active_line_width = 26, track_line_width = 22, start_delay_sec = 0.0) => {
                    ...
                    if (!window._pie_chart_3_obj) {
                        ...
                        window._pie_chart_3_obj = { canvas, tex, lastVal: -1 };
                    }

                    let pieObj = window._pie_chart_3_obj;
                    let colorStr = typeof pie_3_color === "string" ? pie_3_color : rgba_color_string(pie_3_color);
                    ...
                    if (pieObj.lastVal !== currentDisplayVal) {
                        ...
                        // Draw radial stripes from center to outside
                        let total_stripes = 60;     // Configurable density (default 160 stripes in 360 degree)
                        let stripe_thickness = 8;  // Thickness of each stripe
                        ...
                        // Percentage text inside pie chart
                        ctx.font = "400 60px 'Segoe UI', Arial, sans-serif";
                        ctx.fillStyle = rgba_color_string(dial_indicator_color);//colorStr; (colorStr let the percentage color to be same with the color of doughnut line of chart)
                        ctx.textAlign = "center";
                        ctx.textBaseline = "middle";
                        ctx.fillText(Math.round(currentPercentage) + "%", cx, cy);
                ...
                // Draw 60% Animated Pie Chart at 6 o'clock position (between dial center and 9 o'clock hour indicator):
                let pie_3_target_pct = 35;
                let pie_3_px = 6.0;                  // Position X (at 3 o'clock side)
                let pie_3_py = 0.0;                   // Position Y
                let pie_3_width = 8.5;               // Chart width
                let pie_3_height = 8.5;              // Chart height
                let pie_3_self_angle_deg = 90;        // Rotation around chart's own center
                let pie_3_dial_center_angle_deg = 0; // Rotation around center of dial (Z-axis)
                let pie_3_anim_duration = 6.6;        // Animation duration in seconds (e.g., 2.0s, 3.5s, 0.5s)
                let pie_3_active_line_width = 35;     // Width of the active doughnut bar arc (increase e.g. 36 or 44 for thicker bar)
                let pie_3_track_line_width = 31;      // Width of the background track ring
                let pie_3_start_delay = 6.0;          // Start delay in seconds in 22s loop cycle
                try { draw_pie_chart_3_canvas_texture(pie_3_target_pct, pie_3_px, pie_3_py, pie_3_width, pie_3_height, pie_3_self_angle_deg, pie_3_dial_center_angle_deg, pie_3_anim_duration, pie_3_active_line_width, pie_3_track_line_width, pie_3_start_delay); } catch(e) { console.error('Error in pie 3:', e); }

            }

// above for pie chart area
    ...
    function make_drawer(name, slider_count, args, sim_slider_index) {
        let container = typeof name === "string" ? document.getElementById(name) : name;
        if (!container) return [];
        
        let containerName = typeof name === "string" ? name : (container.id || "hero");
        
        // Check if drawer canvas already exists to prevent duplicate creation
        if (container.querySelector(".canvas_container") || container.querySelector("canvas")) {
            console.log("Drawer canvas already exists for " + containerName);
            return [container.drawer];
        }
        
        let ret = [];

        let drawer = new Drawer(container, containerName);
        drawer.sliders = [];
        ret.push(drawer);
        ...
        for (let i = 0; i < slider_count; i++) {
            let slider_el = typeof name === "string" ? document.getElementById(name + "_sl" + i) : document.getElementById((container.id || "hero") + "_sl" + i);
            if (!slider_el) continue;
            
            // Check if slider already exists to prevent duplicate creation
            if (slider_el.querySelector(".slider_container")) {
                console.log("Slider already exists for " + (container.id || "hero") + "_sl" + i);
                continue;
            }
            
            let slider = new Slider(slider_el, function (x) {
                if (i == 0)
                    drawer.set_arg0(x);
                else if (i == 1)
                    drawer.set_arg1(x);
            ...
            drawer.sliders.push(slider);
            ret.push(slider);
        }

        if (sim_slider_index != undefined && ret[sim_slider_index + 1])
            drawer.set_sim_slider(ret[sim_slider_index + 1]);

        return ret;
    }

    window.createWatchWithSliders = make_drawer;

    function initWatchDrawers() {
    ...
    window.createWatchDrawer = function(container, mode) {
        if (!container) return null;
        if (container.querySelector(".canvas_container") || container.querySelector("canvas")) {
            console.log("Drawer canvas already exists for container");
            return container.drawer;
        }
        let drawer = new Drawer(container, mode || "hero");
        return drawer;
    };

    if (window.autoInitializeWatch !== false) {
        if (document.readyState === "loading") {
            document.addEventListener("DOMContentLoaded", initWatchDrawers);
        } else {
            initWatchDrawers();
        }
    }
