...
export default function MechanicalWatch() {
  ...
  const handleToggleIsolate = (isolate: boolean) => {
    setIsIsolated(isolate);
    (window as any).__watchFilterVisible = isolate;
  };
  ...
  useEffect(() => {
    ...
    (window as any).__watchFilterVisible = false;
    ...
    const loadScripts = async () => {
      ...
      if (!(window as any)._watchScriptPromise) {
        (window as any)._watchScriptPromise = new Promise<void>((resolve) => {
          ...
          baseScript.src = `${origin}/js/base.js?v=15`;
          document.body.appendChild(baseScript);

          baseScript.onload = () => {
            const watchScript = document.createElement('script');
            watchScript.src = `${origin}/js/watch.js?v=15`;
